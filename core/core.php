...
class Core {
	...
	public function pro_btn( $pro_link = '', $btn_title = 'Pro', $additional_classes = '', $additional_css = '', $return = false ) {
		...
		$pro_link = ! empty( $pro_link ) ? $pro_link : $this->plugin_info['pro_link'];
		?>
		<a style="<?php echo esc_attr( $additional_css ); ?>" target="_blank" class="ms-2 btn gpls-permium-btn-wave btn-primary pe-4 position-relative <?php echo esc_attr( $additional_classes ); ?>" href="<?php echo esc_url_raw( $pro_link ); ?>">
			<span class="pro-title"><?php printf( esc_html__( '%s' ), $btn_title ); ?></span><span class="gpls-permium-btn-shake">&#187;</span>
			<div class="wave"></div>
		</a>
		<?php
		if ( $return ) {
	...
	public function new_keyword( $title = 'new', $return = true ) {
		...
		?>
		<span style="text-align: center;color: #fff;padding: 1px 4px;background: #b3a8e1;font-size: 15px;font-weight: 700;border-radius: 4px;" class="<?php echo esc_attr( $this->plugin_info['prefix'] . '-new-keyword' ); ?> ms-1"><?php echo wp_kses_post( ! empty( $title ) ? $title : esc_html__( 'New' ) ); ?></span>
		<?php
		if ( $return ) {
			return ob_get_clean();
		}
